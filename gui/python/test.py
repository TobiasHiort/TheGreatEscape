...
PLAYER_SCALE = 0.5 # automate this depending on image res. min 1, max 2?
...
for row in range(MAPHEIGHT):
    for column in range(MAPWIDTH):
        pygame.draw.rect(mapSurface, colors[mapMatrix[row][column]], (math.floor(column*TILESIZE+((907-2*PADDING_MAP)/(2))-((MAPWIDTH*TILESIZE)/2)+PADDING_MAP), (row*TILESIZE+((713-1*PADDING_MAP)/(2))-((MAPHEIGHT*TILESIZE)/2)), TILESIZE, TILESIZE)) 
